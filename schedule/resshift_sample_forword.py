...
# 返回公式需要的东西
    ...
    def sample(self, noisy_image, cond, mask, content):

        x_t = self.prior_sample(cond, noisy_image)

        for time_step in reversed(range(20)):
            print("time_step: ", time_step)
            t = x_t.new_ones([noisy_image.shape[0], ], dtype=th.long) * time_step

            inputs_norm = self._scale_input(x_t, t)
            model_output = self.denoiser(th.cat((inputs_norm, cond, mask, content), dim=1), t)
            ...
            x_t = model_mean + nonzero_mask * th.exp(0.5 * model_log_variance) * noise
        x_0 = x_t
        return x_0
