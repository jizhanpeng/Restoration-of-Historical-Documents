...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, x_T, cond, mask, content, pre_ori='False'):
        """
        Algorithm 2.
        """
        x_t = x_T
        ...
        for time_step in reversed(range(self.T)):
            ...
            if pre_ori == 'False':
                ...
                if time_step > 0:
                    ori = self.model(torch.cat((x_t, cond_, mask, content), dim=1), t)
                    ...
                    x_t = self.model(torch.cat((x_t, cond_, mask, content), dim=1), t)
        x_0 = x_t
        return x_0
