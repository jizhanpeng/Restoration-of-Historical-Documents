...
class DocDiff(nn.Module):
    ...
    def forward(self, x, x_, img, mask, content, t, diffusion):
        noisy_image = diffusion.q_sample(x, x_, t)
        noisy_image = diffusion._scale_input(noisy_image, t)
        x__ = self.denoiser(torch.cat((noisy_image, x_.clone().detach(), mask.clone().detach(), content.clone().detach()), dim=1), t)
        return x_, x__, noisy_image
